...
def make(url):
    ...
    # Request
    req_t = p.select("table")[0].select("tbody tr")
    ...
    response_table = p.select("table")[1].select("tbody tr")
    ...
    sub_elem = u""
    if len(p.select("table")) > 1:
        for i, t in enumerate(p.select("table")[2:]):
            sub_struct = p.select("p")[i+3].text
